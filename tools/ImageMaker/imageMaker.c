...
int main(int argc, char* argv[]) {
    ...
    // Disk.img 파일을 생성
    if ((iTargetFd = open("build/OS.img", O_RDWR | O_CREAT | O_TRUNC | S_IREAD | S_IWRITE)) == -1) {
    ...
    // 파일 크기를 섹터 크기인 512바이트로 맞추기 위해 나머지 부분을 0x00 으로
    // 채움
...
/**
 *  소스 파일(Source FD)의 내용을 목표 파일(Target FD)에 복사하고 그 크기를
 * 되돌려줌
 */
int CopyFile(int iSourceFd, int iTargetFd) {
    int iSourceFileSize;
